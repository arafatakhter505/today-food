...
// Fetch food lists data for the previous day from the database
$previousDayFoodLists = getFoodListsForPreviousDay($pdo);
...
/**
 * Fetch food lists for the previous day
 *
 * @param PDO $pdo
 * @return array
 */
function getFoodListsForPreviousDay($pdo) {
    $yesterday = date('Y-m-d', strtotime('yesterday'));
    $sql = "SELECT food_lists.item, users.full_name 
            FROM users 
            JOIN food_lists ON food_lists.user_id = users.id 
            WHERE food_lists.date = :date";
...
<body>
    ...
    <section class="mx-auto max-w-7xl p-5 sm:p-6 lg:p-8 flex items-start gap-10 justify-between md:flex-row flex-col">
        ...
        <!-- Previous Day Food List Table -->
        <div class="md:w-[1/2] w-full">
            <h2 class="text-lg font-semibold text-gray-700 mb-4">Previous Day's Food List</h2>
            <table class="min-w-full bg-white border border-gray-300 shadow-md">
                ...
                <tbody>
                    <?php if (!empty($previousDayFoodLists)): ?>
                        <?php foreach ($previousDayFoodLists as $food): ?>
                        <tr class="border-t">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="2" class="py-2 px-4 text-gray-700 text-center">No food items added for yesterday.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <!-- Today's Food List Table -->
        <div class="md:w-[1/2] w-full">
        <h2 class="text-lg font-semibold text-gray-700 mb-4">Today Food List</h2>
            <table class="min-w-full bg-white border border-gray-300 shadow-md">
                <thead>
                    <tr class="bg-gray-200">
                        <th class="py-2 px-4 text-left text-gray-600 font-semibold">Name</th>
